 ...
 <?php
		if(isset($_POST["send_s"])){
			...
			if($s=="租借中"){
				...
				$result=mysqli_query($link,"UPDATE `BORROW` SET b_employee='$EMPLOYEE',b_date='$d' WHERE b_a_id='$ID'");
				$result=mysqli_query($link,"UPDATE `APPLY` SET a_status='$s' WHERE a_id='$ID'");
				?>
		...
		if(isset($_POST["send_r"])){
			...
			$EMPLOYEE=$_SESSION["account"];
			$return=$_POST["RETURN"];
			$result=mysqli_query($link,"UPDATE `BORROW` SET b_return_date='$return' WHERE b_a_id='$ID'");
			?>
					<script >alert("修改成功!");</script>
				<?php
			header('Refresh:0.1; url=apply.php');
	...
	<div class="container">
		<div class="row centered mt mb">
			...
			<span style="font-family:Microsoft JhengHei;color:#000000;">
				<h2>修改租借單</h2>
				<form method="post" action="mod_bow.php">
				<table align="center" style='border:3px #00DD00 dashed;font-size:20px;' cellpadding='10' border='1'>
				<tr><td>租借單編號</td><td>租借人</td><td>租借日</td><td>實還日期</td><td>預還日期</td><td>備註</td><td>狀態</td><td>經手人</td></tr>
				<?php
					$BID=$_GET["ID"];
					$_SESSION["BID"]=$BID;
					$name=$_GET["name"];
					$date=$_GET["date"];
					$return=$_GET["return"];
					$PREreturn=$_GET["PREreturn"];
					$employee=$_GET["employee"];
					$remark=$_GET["remark"];

					echo "<tr><td>".$BID."</td><td>".$name."</td><td><input type='date' name='bow_day' ></td><td><input type='datetime-local' name='RETURN' ><br><input type='submit' class='btn btn-primary' name='send_r' style='font-family:Microsoft JhengHei;width:60px;height:30px;font-size:0.4cm;' value='修改'/></td><td>".$PREreturn."</td><td>".$remark."</td><td>";
					echo "<select name='status'><option value='領取中'>領取中</option><option value='租借中'>租借中</option><option value='已歸還'>已歸還</option></select><br><input type='submit' class='btn btn-primary' name='send_s' style='font-family:Microsoft JhengHei;width:60px;height:30px;font-size:0.4cm;' value='修改'/></td><td>".$employee."</td></tr>"
				?>
				</table>
			</form>
			</span>
